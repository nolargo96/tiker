...
class ConfigManager:
    ...
    def _default_config(self) -> Dict[str, Any]:
        """デフォルト設定"""
        return {
            'data': {'default_period_days': 365, 'min_trading_days': 250, 'buffer_multiplier': 1.5},
            'technical': {'ema_short': 20, 'ema_long': 50, 'sma_long': 200, 'rsi_period': 14},
            'chart': {'figure_size': [16, 9], 'dpi': 100, 'panel_ratios': [3, 1]},
            'directories': {'charts': './charts', 'data': './data', 'reports': './reports'},
            'naming': {
...
class TechnicalIndicators:
    ...
    @staticmethod
    def calculate_moving_averages(df: pd.DataFrame, config: ConfigManager) -> pd.DataFrame:
        """移動平均線を計算"""
        df_calc = df.copy()
...
class StockDataManager:
    ...
    def fetch_stock_data(self, ticker: str, days: Optional[int] = None) -> Tuple[bool, pd.DataFrame, str]:
        """株価データを取得"""
        if days is None:
        ...
        self.logger.info(f"データ取得: {ticker}, 期間: {start_date.strftime('%Y-%m-%d')} - {end_date.strftime('%Y-%m-%d')}")
        ...
        try:
            ...
            if df.empty:
                ...
                if not info or 'regularMarketPrice' not in info:
                    return False, pd.DataFrame(), f"{ticker} は有効な米国株ティッカーではありません。"
                else:
                    return False, pd.DataFrame(), "データが取得できませんでした。データ不足のため簡易分析。"
            
            if len(df) < min_trading_days:
                self.logger.warning(f"{ticker} のデータが{min_trading_days}日分ありません。取得: {len(df)}日分")
...
class ChartGenerator:
    ...
    def create_chart(self, df: pd.DataFrame, ticker: str, date_str: str) -> Tuple[bool, str]:
        """チャートを生成・保存"""
        try:
            # ディレクトリ設定
            ...
            # ファイル名生成
            chart_pattern = self.config.get('naming.chart_pattern', '{ticker}_chart_{date}.png')
            ...
            # 追加プロット
            addplots = [
                mpf.make_addplot(df['EMA20'], color=colors.get('ema_short', 'blue'), width=0.7, panel=0),
                mpf.make_addplot(df['EMA50'], color=colors.get('ema_long', 'orange'), width=0.7, panel=0),
                mpf.make_addplot(df['SMA200'], color=colors.get('sma_long', 'purple'), width=0.7, panel=0),
            ]
...
class StockAnalyzer:
    ...
    def analyze_stock(self, ticker: str, date_str: Optional[str] = None) -> Tuple[bool, str]:
        """統合株式分析を実行"""
        if date_str is None:
        ...
        # 3. チャート生成
        chart_success, chart_result = self.chart_generator.create_chart(df_analysis, ticker, date_str)
        ...
        # 4. データ保存
        data_pattern = self.config.get('naming.data_pattern', '{ticker}_analysis_data_{date}.csv')
    ...
    def _display_latest_data(self, df: pd.DataFrame, ticker: str):
        ...
        if not pd.isna(latest['BB_upper']):
            print(f"ボリンジャーバンド: ${latest['BB_lower']:.2f} - ${latest['BB_upper']:.2f}")
