...
class CompetitorAnalysis:
    ...
    def get_competitor_data(self, ticker: str, period_days: int = 365) -> Dict[str, Any]:
        """
        指定銘柄の競合企業データを取得
        ...
        for symbol in all_tickers:
            success, df, message = self.data_manager.fetch_stock_data(symbol, period_days)
            ...
            if success and not df.empty:
                ...
                results['data'][symbol] = {
                    'name': competitor_info['descriptions'].get(symbol, symbol) if symbol != ticker else competitor_info['name'],
                    ...
                    'price_vs_ema20': (latest_price - df['EMA20'].iloc[-1]) / df['EMA20'].iloc[-1] * 100 if 'EMA20' in df.columns else None,
    ...
    def _calculate_relative_performance(self, results: Dict[str, Any]) -> None:
        ...
        for symbol, data in valid_data.items():
            if data['volatility_pct'] > 0:
                risk_adjusted[symbol] = data['total_return_pct'] / data['volatility_pct']
        
        results['performance_comparison'] = {
            'return_ranking': sorted_returns,
            'risk_adjusted_ranking': sorted(risk_adjusted.items(), key=lambda x: x[1], reverse=True),
            'sector_average_return': np.mean(list(returns.values())),
            'sector_median_return': np.median(list(returns.values())),
            'target_vs_sector': returns.get(results['target_ticker'], 0) - np.mean(list(returns.values()))
        }
...
"""
        ...
        for i, (symbol, return_pct) in enumerate(data['performance_comparison']['return_ranking'], 1):
...
"""
        ...
        if 'error' not in target_data:
            rank = next(i for i, (s, _) in enumerate(data['performance_comparison']['return_ranking'], 1) if s == ticker)
...
"""
        ...
        if 'error' not in target_data:
            risk_adjusted_rank = next(i for i, (s, _) in enumerate(data['performance_comparison']['risk_adjusted_ranking'], 1) if s == ticker)
...
"""
    ...
    def analyze_all_portfolio_competitors(self, period_days: int = 365) -> Dict[str, str]:
        """
        ポートフォリオ全9銘柄の競合分析を実行
