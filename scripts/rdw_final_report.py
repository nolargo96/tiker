...
print(f"- **企業ビジョンミッション**: 宇宙インフラストラクチャのミッションクリティカルなコンポーネントとシステムの開発製造を通じて、人類の宇宙活動を支援")
print(f"- **主要事業セグメント**: 宇宙インフラ展開式構造物(45%)、太陽電池アレイ電力システム(35%)、先端製造材料(20%)")
print(f"- **主力製品サービス**: Roll-Out Solar Arrays (ROSA)、宇宙用3Dプリンティング、マイクログラビティ研究プラットフォーム、デジタルエンジニアリングサービス")
...
print("\n**Round 1: エントリーシグナル検証**")
print("TECHFUND: 株価は20日EMAでサポートされ、RSI50付近で推移。過度な売り込みからは回復したが、明確な上昇トレンドはまだ見えない。この価格帯での滞留は、ファンダメンタルズの改善待ちか？")
print("FUND: 直近の決算では売上高がガイダンスを上回り、新規契約の獲得も順調。ただし、利益率改善は遅れており、市場は黒字化時期を見極めようとしている。現在の株価は業績改善期待と実現リスクのバランス点。")
print("MACRORISK: NASA予算は前年比7%増、Space Force予算も20%増と宇宙セクターへの資金流入は継続。しかし、小型株全般への資金流出も続いている。このギャップをどう評価すべきか？")
print("RISK: セクターテーマは強いが、個別株リスクは高い。分散投資の観点から、宇宙ETF(UFO、ROKT)との組み合わせを推奨。RDW単独では総資産の1%以下に抑え、セクター全体で3-5%の配分が妥当。")

print("\n**Round 2: 下値目処の確定**")
print(f"TECH: 直近安値${recent_low:.2f}が第一サポート。その下は52週安値${low_52w:.2f}が強力な心理的節目。フィボナッチリトレースメントでは${fib_382:.2f}(38.2%)、${fib_50:.2f}(50%)が注目レベル。出来高分析では$2.00-2.20に大口の買い支えあり。")
print("FUND: 時価総額が$100M（株価約$1.50）を割ると機関投資家の投資対象外となるリスク。純資産価値ベースでは$2.00が下限。競合他社のEV/Sales倍率1倍を当てはめると$2.20-2.50が妥当な下値。")
print("MACRO: SPACブーム崩壊で多くのSPAC株が$1-2に収束。ただし、実業を持つRDWは下値余地限定的。市場全体が20%調整してもRDWの下値は$2.00前後と想定。")
print("RISK: VaRモデルでは95%信頼区間で$1.80が下限。過去の最大ドローダウン70%を現在価格に適用すると$0.85だが、これは極端なテールリスク。現実的には$2.00を防衛ラインとし、そこを割れたら撤退。")

print("\n**Round 3: 上値目標の設定**")
print(f"TECH: 第一目標は直近高値${recent_high:.2f}。その上は年初来高値$4.50、さらに52週高値${high_52w:.2f}がターゲット。チャートパターンから、$3.00突破後は$4.00-4.50への上昇が期待できる。長期では$6-8レンジも視野。")
print("FUND: 2025年売上$200M、2027年黒字化シナリオで、PSR2倍なら時価総額$400M（株価$6）。楽観シナリオで2027年売上$300M、営業利益率10%、PER25倍なら株価$11-12も可能。ただし実現確率は30%程度。")
print("MACRO: 宇宙経済は2030年までに$1兆規模に成長予測。RDWがシェア0.1%獲得なら売上$1B可能。ただし、競争激化と統合再編も予想され、M&Aターゲットになる可能性も。買収プレミアム込みで$8-10もあり得る。")
print("RISK: 各目標の実現確率：1年後$4.00(50%)、$5.00(30%)、$6.00(15%)、$8.00以上(5%)。期待値は$4.20。リスク調整後リターンを考慮すると、$4.50での部分利確が賢明。")

print("\n**Round 4: 段階的エントリー戦略**")
print(f"全員の議論を統合し、以下の段階的エントリー戦略を策定：")
print(f"第1段階（現在-$2.70）: 現在の株価${current_price:.2f}付近は様子見。20日EMA(${current_ema20:.2f})での明確な反発確認後、総資産の0.5%でテストポジション構築。")
print(f"第2段階（$2.50-2.70）: RSI40以下への下落時に総資産の0.5%追加。200日SMA接近も買いシグナル。")
print(f"第3段階（$2.20-2.50）: 本格的な買い場。総資産の1%を投入。52週安値更新の投げ売りを狙う。")
print(f"緊急買い（$2.00-2.20）: 想定外の下落時の最終防衛ライン。総資産の0.5%で打診買い。")
print(f"時間軸は3-6ヶ月。急がず、株価の動きを見ながら柔軟に対応。")

print("\n**Round 5: 撤退損切り基準**")
print("TECH: $2.00を終値で下回ったら技術的に売り。また、50日EMAが200日EMAを下抜け（デッドクロス）したら中期トレンド悪化で撤退検討。")
print("FUND: 四半期決算で2期連続の売上未達、または主要顧客（NASA等）との契約喪失で投資前提崩壊。経営陣の大量離職も危険信号。")
print("MACRO: 宇宙予算の10%以上の削減、または宇宙開発に関する重大な規制強化（デブリ規制等）で産業全体の成長鈍化。")
print("RISK: ポジション全体で初期投資額の25%損失（加重平均取得価格から25%下落）で機械的に半分売却。40%損失で全売却。感情を排し、ルールを厳守。")

print("\n**Round 6: 保有期間と出口戦略**")
print("基本保有期間は3-5年。長期的な宇宙産業の成長を享受する戦略。")
print("利益確定ルール：取得価格の2倍（100%上昇）で1/3売却、3倍で更に1/3売却、残り1/3は長期保有。")
print("定期レビュー：四半期決算ごとに事業進捗を確認。年1回（毎年1月）にポートフォリオ全体でのウェイト調整。")
print("M&A対応：買収提案があれば、プレミアムと買収企業の質を評価。現金買収なら受け入れ、株式交換なら交換先企業を精査。")
...
print(f"**推奨理由**: RDWは宇宙インフラ分野で確固たる技術と顧客基盤を持つ。現在の株価${current_price:.2f}は上場時の過熱感から調整済みだが、まだ下値リスクあり。$2.50近辺への調整を待ち、段階的にエントリーすることで、リスクを抑えつつ中長期の成長を狙う。宇宙産業の構造的成長は継続するが、個別企業の選別が進む局面。小型株のため、ポジションサイズは必ず守ること。")
