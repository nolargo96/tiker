...
current_rsi = df['RSI'].iloc[-1] if 'RSI' in df.columns and not pd.isna(df['RSI'].iloc[-1]) else 45
current_ema20 = df['EMA20'].iloc[-1] if 'EMA20' in df.columns and not pd.isna(df['EMA20'].iloc[-1]) else current_price
current_ema50 = df['EMA50'].iloc[-1] if 'EMA50' in df.columns and not pd.isna(df['EMA50'].iloc[-1]) else current_price
current_sma200 = df['SMA200'].iloc[-1] if 'SMA200' in df.columns and not pd.isna(df['SMA200'].iloc[-1]) else current_price * 0.9
...
print(f"- **企業ビジョンミッション**: クリーンで手頃な太陽光発電を通じて、持続可能なエネルギーの未来をリードする。")
print(f"- **主要事業セグメント**: 太陽光発電モジュールの製造販売が大部分を占める。")
print(f"- **主力製品サービス**: テルル化カドミウム(CdTe)薄膜太陽電池モジュール。米国内製造に強みを持つ。")
...
print("\n**Round 1: エントリーシグナル検証**")
print("TECHFUND: 株価は200日SMAを割り込み、テクニカル的には明確な「売り」シグナルだ。この下落は、ファンダメンタルズの悪化を織り込む動きか、それとも絶好の買い場を提供しているのか？")
print("FUND: 後者、絶好の買い場と見る。株価下落はマクロ要因やセクター全体のセンチメント悪化が主因であり、FSLR固有のファンダメンタルズは依然として堅固。巨額の受注残は変わらず、IRAの恩恵もこれから本格化する。市場の悲観は過剰だ。")
print("MACRORISK: 次期大統領選で共和党が勝利し、IRAが縮小撤廃されるシナリオの発生確率と、その場合の株価へのインパクトをどう見積もるか？")
print("RISK: IRA撤廃リスクは現時点で20-30%と見積もる。もし現実となれば、$100割れも覚悟すべき。このテールリスクを管理するため、FSLRへの投資はポートフォリオの一部に留め、選挙が近づくにつれてヘッジとしてPUTオプションの購入も検討すべきだ。")

print("\n**Round 2: 下値目処の確定**")
print(f"TECH: まずは現在のサポートゾーンである$140-$150。ここを割れると、次の節目は52週安値の${low_52w:.2f}付近となる。その下は心理的節目の$120。")
print(f"FUND: IRAの恩恵を割り引いても、現在の受注残だけで理論株価は$150以上と計算できる。現在の株価は明らかに売られ過ぎ。$130-$140は極めて魅力的な買いゾーン。")
print("MACRO: 中国製パネルの供給過剰でセクター全体のセンチメントが悪化し続ければ、$120台までの下落も視野に入る。ただし、米国内の強固な需要がそれ以上の下落を防ぐだろう。")
print("RISK: 過去のドローダウンを見ると、$135が統計的な下限。IRA撤廃のヘッドラインが出た場合は$100も。全員の意見を統合すると、確度の高いサポートゾーンは$135-$145。")

print("\n**Round 3: 上値目標の設定**")
print(f"TECH: まずは200日SMA(${current_sma200:.2f})の奪還が目標。その後はフィボナッチリトレースメントの半値戻しである${fib_50:.2f}。長期では過去の高値${high_52w:.2f}を目指す展開。")
print("FUND: 2026年の予想EPS $25にPER15倍（保守的）を適用しても、目標株価は$375。現在の株価からのアップサイドは非常に大きい。1年後の目標は$220と見る。")
print("MACRO: AIのデータセンター向け電力需要の急増が、新たな太陽光発電の需要を生み出している。このトレンドが加速すれば、FSLRの市場はさらに拡大し、株価$300以上を目指す展開も。")
print("RISK: 1年後の目標株価$220の達成確率は60%。$250到達は40%と見る。IRA政策が維持されることが大前提。現在の株価水準はリスクリワードレシオが極めて高い。")

print("\n**Round 4: 段階的エントリー戦略**")
print(f"全員の議論を統合し、以下の段階的エントリー戦略を策定：")
print(f"第1段階（$145-155）: 現在の価格帯。打診買いとして初期ポジションの30%（総資産の0.6%）でエントリー。")
print(f"第2段階（$135-145）: 確度の高いサポートゾーン。ここで残りの70%（総資産の1.4%）を追加し、平均取得単価を下げる。")
print("時間軸は2-3年。IRA政策の動向を注視しながら、逆張り的にポジションを構築する。")

print("\n**Round 5: 撤退損切り基準**")
print("TECH: $130を明確に終値で下回り、下落トレンドが継続する場合。")
print("FUND: 2四半期連続で受注残高が市場予想を下回り、長期的な成長ストーリーに疑念が生じた場合。")
print("MACRO: 米国大統領選挙の結果、IRAの大幅な見直しや撤廃が決定した場合。")
print("RISK: 初期投資額に対して25%の損失で損切り。政策リスクが顕在化した場合は、損失率に関わらず速やかにポジションを解消する。")

print("\n**Round 6: 保有期間と出口戦略**")
print("コア保有期間は2025年末までとし、大統領選の結果とIRA政策の行方を見極める。")
print("利益確定：目標株価$220に到達した場合、ポジションの半分を利益確定。$250以上ではさらに半分を確定し、残りを長期保有。")
print("政策リスク管理：2024年後半にかけて、選挙に関する報道に注意を払い、リスクが高まったと判断した場合は利益確定または損切りを前倒しで行う。")
...
print(f"**推奨理由**: FSLRの株価は現在、テクニカル的には弱気だが、ファンダメンタルズは極めて堅固。このギャップは絶好の逆張り投資機会を提供している。現在の価格${current_price:.2f}から$135にかけて段階的に買い下がる戦略を推奨。最大の懸念材料は米国の政策変更リスクであり、ポジションサイズを管理し、損切りを徹底することが成功の鍵となる。")
print("**次回レビュータイミング**: 次回四半期決算発表後、および米国大統領選挙に関する重要な報道があった時点。")
