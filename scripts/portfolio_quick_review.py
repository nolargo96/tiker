...
from src.analysis.unified_analyzer import analyze_portfolio
...
def quick_review(custom_config=None, date_str=None):
    ...
    if results["expert_scores"]:
        ...
        for i, (ticker, scores) in enumerate(sorted_scores, 1):
            ...
            print(
                f"  {i}. {ticker} ({allocation}%): {scores['OVERALL']:.1f}★ - {recommendation['action']}"
            )
...
def main():
    ...
    parser.add_argument(
        "--conservative", action="store_true", help="保守的ポートフォリオを使用"
    )
    parser.add_argument(
        "--aggressive", action="store_true", help="積極的ポートフォリオを使用"
    )
    parser.add_argument(
        "--custom", type=str, help="カスタム設定 (例: TSLA:40,FSLR:30,ASTS:20,OKLO:10)"
    )
