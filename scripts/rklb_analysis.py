...
try:
    ...
    # チャート設定
    ax1.set_title(f'{ticker} Daily Chart (1 Year) - Data as of {today_str} JST', fontsize=16, fontweight='bold') # <--- ticker変数を反映
    ...
    # チャートを保存
    chart_filename = f'rklb_chart_{today_str}.png' # <--- 変更点
    plt.savefig(chart_filename, dpi=150, bbox_inches='tight')
    print(f"\nチャートを{chart_filename}に保存しました: {chart_filename}") # <--- 変更点
    ...
    # データをCSVに保存（分析用）
    csv_filename = 'rklb_data.csv' # <--- 変更点
    df.to_csv(csv_filename)
    print(f"\nデータを{csv_filename}に保存しました") # <--- 変更点
