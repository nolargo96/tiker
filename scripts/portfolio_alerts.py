...
class PortfolioAlerts:
    ...
    def _check_ticker_alerts(
        self, ticker, current_price, prev_price, change_pct, levels, allocation
    ):
        """個別銘柄のアラートチェック"""
        alerts = []
    ...
    def display_alerts(self, alerts):
        ...
        print(f"\n📊 アラートサマリー:")
        print(
            f"  - 緊急: {len([a for a in all_alerts if a['priority'] == 'CRITICAL'])}件"
        )
        print(f"  - 重要: {len([a for a in all_alerts if a['priority'] == 'HIGH'])}件")
        print(
            f"  - 通常: {len([a for a in all_alerts if a['priority'] == 'MEDIUM'])}件"
        )
...
def main():
    ...
    parser.add_argument(
        "--save-history", action="store_true", help="アラート履歴を保存"
    )
